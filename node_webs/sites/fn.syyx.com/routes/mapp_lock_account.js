...
请求:
    参数：
        ...
        appid
    示例：
        http://functions.syyx.com/mapp_lock_account?account=abcd&appid=54698721
        ...
        ms.account_mgr.check_appid(account, function(result, userappid){
            ...
            ms.account_mgr.lock_account(userid, account, function(err){
                ...
                ms.account_mgr.lock_gameaccount(userid, account, function(result){
                    ...
                    var isok = result == 0;
                    if(isok){
                        ms.log_mgr.add_lockgameaccountlog(userid, account, 29, isok)

                        var ip = ms.u2.get_req_ip(req)
                        ms.log_mgr.add_lockoperatelog(0,' ', ip, userid, account, '用户通过掌尚游锁定账号', 29)

                        ms.log_mgr.add_applog(account, appid, 3, "用户通过掌尚游锁定账号")
                    }
                })                
            })                      
        })
