...
请求:
    参数：
        ...
        mobilenumber
    示例：
        http://functions.syyx.com/kf_lock_request?account=abcd&mobilenumber=13322111111
    ...
    ms.account_mgr.get_userid(account, function(err, userid){
        ...
        ms.account_mgr.check_mobile(account, mobile, function(result){            
            ...
            ms.account_mgr.check_mobile_right(account, function(result, changepswd, selfhelp, nofitylogin){
                ...
                ms.sms_check.create(mobile, 'kf_lock', ip, function(err, validatecode){
                    if(err !=0 ){
                        if(err = 2)
                            res.send({ retcode : 1 })
                        else
                    ...
                    var msg = '您好，您的账号'+ account +'在'+ date+'进行自助封停，验证码为：'+ validatecode+'。本信息免费'
