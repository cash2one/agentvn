...
exports.action = function(req, res) {
    ...
    var ret = request_reg(req, function(err, result) {
        ...
        var ret = register_ok_posttreat(req, res)
        if (!ret) {
            res.send({ type : 'no_card', info : [ account ] })
            ms.u.log(ms.u.format('[%s] [%s] [%s] register ok, no card need', account, client_ip, page_name))
...
//--------------------------------------------------------------------------------------------------------
function register_ok_posttreat(req, res) {
    var account   = req.body.account_name
    var page_name = req.body.page_name
    var client_ip = ms.u2.get_req_ip(req)
    ...
    var ret = ms.card_mgr.apply_card(account, client_ip, page_name, function(err, card) {
        if (!card) {
            res.send({ type : 'no_card', info : [ account ] })
            ms.u.log(ms.u.format('[%s] [%s] [%s] register ok, but no card, %s', account, client_ip, page_name, err))

            return
        }

        res.send({ type : 'ok', info : [ account, card ] })
        ms.u.log(ms.u.format('[%s] [%s] [%s] register ok', account, client_ip, page_name))
    })
