...
getflash('http://r.syyx.com/events/p057/prize.swf', 284, 55, "#prize")

function img_focus() {
    var imgfocus = 0
    for (var i = 5; i--;) {
        $("#img_focus_tab a").eq(i).click(function(a) {
            return function() {
                if(imgfocus == 0) {
                    $("#img_focus_img div").eq(imgfocus).html("")
                }
                if(a == 0) {
                    getjwplayer("http://v.nycs.syyx.com/nycs/flv/TVCforSE_676X350_201209.flv", 676, 350, "#tvc")
                }
                $("#img_focus_tab a").eq(imgfocus).removeClass("check_" + imgfocus)
                $(this).addClass("check_" + a)
                $("#img_focus_img div").eq(imgfocus).fadeOut("fast")
                var focus = $("#img_focus_img div").eq(a)
                focus.fadeIn("fast")
                imgfocus = a;
            }
        }(i))
