...
//-------------------------------------------------------------------------------------------
function get_data() {
    get_cross_domain_data(remote_server_url_list.get_map_task, 'get_map_task', { mid : get_url_parm() }, function(data){
        $.each(data, function(index, value){
            //添加页面最后的双table
            if(index == data.length-1) {
                if($.isArray(value.tasklist)) {

                    var table_format = '<table><tr><th>任务名称</th><th>前置任务</th><th>发布人</th><th>要求等级</th><th>任务奖励</th></tr></table>'

                    if(value.tasklist.length < 1) {
                        return                        
                    }
            ...
            $('.lib_nav').append('<a href="'
                + remote_server_url_list.game_lib_map_task
                + '?'
                + value.ID
                + '">'
                + value.Title
