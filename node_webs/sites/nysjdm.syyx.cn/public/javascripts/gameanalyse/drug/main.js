        ...
        success: function(json) {
            ...
            $.each(json.rows,function(i,item){
                $('<option value="'+item.ID+'"> '+$.trim(item.ClassName)+'</option>')
                    .appendTo($('#DrugTypeList'));
            })
             $('#DrugTypeList')[0].selectedIndex = 0;
        }
    });
}
...
function getDrugList(){
    ...
    var classID     = $('#DrugTypeList').children('option:selected').val();
    ...
    if(getType == 1){
        tableName = "Game_DrugCreate";
    }
    else{
        tableName = "Game_DrugDestroy";
    }
    ...
    $.ajax({
        ...
        success:function(itemJson){  
             ...
             $.ajax({
                url:'http://nysjdm.syyx.cn/gameanalyse/game_drug_list?r='+Math.random(),
                type: "get",
                dataType: "json",
                ...
                beforeSend: function() { $("#loading").show(); },
                complete: function() { $("#loading").hide(); },
                success: function(drugJson) {
                    $("#tableDrug thead").remove();
                    $("#tableDrug tbody").remove();
                    var tablehead   = "<thead><tr>";
                    var tablebody   = "<tbody>";
                    var druglen    = drugJson.rows.length;
                        tablehead += '<th>日期</th>'; 
                    for(var drugKey in drugJson.rows[0]){
                        if(itemKey[drugKey]){
                                tablehead += '<th>' + itemKey[drugKey] + '</th>';
                            }
                    }
                    tablehead += "<th>合计</th></tr></thead>";
                    $(tablehead).appendTo("#tableDrug");
                    for(var j = 0; j < druglen; j++)
                    {   
                        var itemSum = 0;
                        tablebody += "<tr class='newrow'><td>"+drugJson.rows[j]["CreateDate"]+"</td>";
                        for(var itemDrugKey in drugJson.rows[0])
                        {
                           
                            if(itemDrugKey != "CreateDate"){
                                var itemCount = drugJson.rows[j][itemDrugKey] == "NULL"?0:drugJson.rows[j][itemDrugKey];
                                itemSum += itemCount;
