...
function getHomeSteadTypeList(){
      $.ajax({
        url:'http://nysjdm.syyx.cn/parameter/config_class_list?r='+Math.random(),
        type: "get",
        dataType: "json",
        data: {ParamsType:"Home"},
        beforeSend: function() { $("#loading").show(); },
        complete: function() { $("#loading").hide(); },
        success: function(json) {
            $('#homesteadTypeList').empty();
            $.each(json.rows,function(i,item){
                $('<option value="'+item.ID+'"> '+$.trim(item.ClassName)+'</option>')
                    .appendTo($('#homesteadTypeList'));
            })
             $('#homesteadTypeList')[0].selectedIndex = 0;
        }
    });
}

function getHomeSteadList(){
    var tableName    = "";
    var itemList     = "";
    var itemKey      = {};
    ...
    var classID      = $('#homesteadTypeList').children('option:selected').val();
    var classIDIndex = $('#homesteadTypeList')[0].selectedIndex;
    ...
    if(classIDIndex < 2){
        if(getType == 1){
            tableName = "Game_HomeSteadCreate";
        }
        else{
            tableName = "Game_HomeSteadDestroy";
        }
    }else{
        if(getType == 1){
    ...
    $.ajax({
        ...
        success:function(itemJson){  
             ...
             $.ajax({
                url:'http://nysjdm.syyx.cn/gameanalyse/game_homestead_list?r='+Math.random(),
                type: "get",
                dataType: "json",
                ...
                beforeSend: function() { $("#loading").show(); },
                complete: function() { $("#loading").hide(); },
                success: function(homesteadJson) {
                    $("#tableHomeStead thead").remove();
                    $("#tableHomeStead tbody").remove();
                    var tablehead   = "<thead><tr>";
                    var tablebody   = "<tbody>";
                    var homesteadlen    = homesteadJson.rows.length;
                        tablehead += '<th>日期</th>'; 
                    for(var homesteadKey in homesteadJson.rows[0]){
                        if(itemKey[homesteadKey]){
                                tablehead += '<th>' + itemKey[homesteadKey] + '</th>';
                            }
                    }
                    tablehead += "<th>合计</th></tr></thead>";
                    $(tablehead).appendTo("#tableHomeStead");
                    for(var j = 0; j < homesteadlen; j++)
                    {   
                        var itemSum = 0;
                        tablebody += "<tr class='newrow'><td>"+homesteadJson.rows[j]["CreateDate"]+"</td>";
                        for(var itemHomeSteadKey in homesteadJson.rows[0])
                        {
                           
                            if(itemHomeSteadKey != "CreateDate"){
                                var itemCount = homesteadJson.rows[j][itemHomeSteadKey] == "NULL"?0:homesteadJson.rows[j][itemHomeSteadKey];
                                itemSum += itemCount;
                    ...
                    tablebody += "</tbody>";
                    $(tablebody).appendTo("#tableHomeStead");
                }
            });
        }
    });
