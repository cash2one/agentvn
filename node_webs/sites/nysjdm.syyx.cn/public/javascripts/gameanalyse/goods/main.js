        ...
        success: function(json) {
            ...
            $.each(json.rows,function(i,item){
                $('<option value="'+item.ID+'"> '+$.trim(item.ClassName)+'</option>')
                    .appendTo($('#goodsTypeList'));
            })
             $('#goodsTypeList')[0].selectedIndex = 1;
        }
    });
}
...
function getGoodsList(){
    ...
    var classID     = $('#goodsTypeList').children('option:selected').val();
    var getType     = $('#InOutTypeList').children('option:selected').val();
    var className   = $.trim($('#goodsTypeList').children('option:selected').text());
    ...
    if( className == "精力" || className == "活力"){
        if(getType == 1)
            tableName = "Game_VirtualCreate";
        else
            tableName = "Game_VirtualDestroy";
     }else{
        if(getType == 1)
    ...
    $.ajax({
        ...
        success:function(itemJson){  
             ...
             $.ajax({
                url:'http://nysjdm.syyx.cn/gameanalyse/game_goods_list?r='+Math.random(),
                type: "get",
                dataType: "json",
                ...
                success: function(goodsJson) {
                    $("#tableGoods thead").remove();
                    $("#tableGoods tbody").remove();
                    var tablehead   = "<thead><tr>";
                    var tablebody   = "<tbody>";
                    var equiplen    = goodsJson.rows.length;
                        tablehead += '<th>日期</th>'; 
                    for(var goodsKey in goodsJson.rows[0]){
                        if(itemKey[goodsKey]){
                                tablehead += '<th>' + itemKey[goodsKey] + '</th>';
                            }
                    }
                    tablehead += "<th>合计</th></tr></thead>";
                    $(tablehead).appendTo("#tableGoods");
                    for(var j = 0; j < equiplen; j++)
                    {   
                        var itemSum = 0;
                        tablebody += "<tr class='newrow'><td>"+goodsJson.rows[j]["CreateDate"]+"</td>";
                        for(var itemGoodsKey in goodsJson.rows[0])
                        {
                           
                            if(itemGoodsKey != "CreateDate"){
                                var itemCount = goodsJson.rows[j][itemGoodsKey] == "NULL"?0:goodsJson.rows[j][itemGoodsKey];
                                itemSum += itemCount;
