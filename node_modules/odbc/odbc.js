...
Database.prototype.processQueue = function () {
  ...
  if (!self.req_queue) self.req_queue = [];
  
  if (self.connected && !self.executing && self.req_queue.length) {
    var currentQuery = self.req_queue[0];
    self.executing = true;
...
Database.prototype.query = function(sql, params, callback) {
  ...
  if (callback == null) {
    callback = params; // no params2 supplied
    params = null;
  }
  ...
  args.push(function (error, rows, morefollowing, result) {
    //check to see if this is the last result set returned
    if (!morefollowing) {
      self.req_queue.shift();
...
/**
 * 
 * We must req_queue the close. If we don't then we may close during the middle of a query which 
