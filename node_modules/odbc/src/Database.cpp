  ...
  //else {
    ...
    do {
      ...
      if (colCount > 0) {
        ...
        while(true)
        {
          ...
          if (ret == SQL_ERROR)  {
            ...
            char errorSQLState[128];
            SQLError(self->m_hEnv, self->m_hDBC, self->m_hStmt,(SQLCHAR *)errorSQLState,NULL,(SQLCHAR *)errorMessage, sizeof(errorMessage), NULL);
            if(strcmp(errorSQLState,"24000") != 0) {
              errorCount++;
              objError->Set(String::New("state"), String::New(errorSQLState));
              objError->Set(String::New("error"), String::New("[node-odbc] SQL_ERROR"));
              objError->Set(String::New("message"), String::New(errorMessage));
      ...
      if (colCount > 0 || ( ret != SQL_SUCCESS && emitCount == 0 )) {
	    ...
	    for (int i = 0,j=0; i < prep_req->ParamCount; i++)
		{
			prm = prep_req->Params[i];
			if ( prm.isoutparam && prm.buffer != NULL)
			{
			  switch (prm.c_type)
				{
				  case SQL_C_CHAR:   result->Set(Integer::New(j), String::New((char *)prm.buffer)); break; 
				  case SQL_C_LONG:   result->Set(Integer::New(j), Number::New(*(int64_t *)prm.buffer)); break;
				  case SQL_C_DOUBLE: result->Set(Integer::New(j), Number::New(*(double  *)prm.buffer)); break;
				  case SQL_C_BIT:    result->Set(Integer::New(j), Boolean::New( ( *buf == '0') ? false : true ));  break;	
				}
			  j++;	
			}
		}
		 ...
		 if (prm = prep_req->Params[i], prm.buffer != NULL)
			{
			  switch (prm.c_type)
				...
				case SQL_C_TYPE_TIMESTAMP: 
                   delete (TIMESTAMP_STRUCT *)prm.buffer; break;
        }
			}
		}
...
void Database::EIO_Query(eio_req *req) {
  ...
  else 
  {
	  ...
	  for (int i = 0; i < prep_req->ParamCount; i++)
    {
		    prm = prep_req->Params[i];
        if(prm.isoutparam)
			     ret = SQLBindParameter(prep_req->dbo->m_hStmt, i+1, SQL_PARAM_OUTPUT, prm.c_type, prm.type, prm.size, 0, prm.buffer, prm.buffer_length, &prep_req->Params[i].length);
		    else
...
Handle<Value> Database::Query(const Arguments& args) {
  ...
  if (args.Length() > 2) 
  {
     ...
     Local<Array> param_info  = Local<Array>::Cast(args[1]);
     prep_req->ParamCount 	  = paramCount = param_info->Length()+1;
     prep_req->Params         = params     = new Parameter[paramCount];

	 //返回值
	 params[0].isoutparam	 = true;
	 params[0].size          = 0;
	 params[0].length        = NULL;
	 params[0].buffer_length = 0;
	 params[0].c_type 		 = SQL_C_LONG;						   
	 params[0].type   		 = SQL_INTEGER;
	 params[0].buffer 		 = new int64_t(0);	  	  
      
	 for (int i = 1; i < paramCount; i++)
     {
      Local<Object> info	    = Local<Object>::Cast(param_info->Get(i-1));
  		Local<Value>  value     = info->Get(String::New("param_value"));
  		params[i].isoutparam	= info->Get(String::New("isoutparam"))->BooleanValue();	
  		params[i].size          = info->Get(String::New("param_length"))->IntegerValue();
  		params[i].length        = NULL;
  		params[i].buffer_length = 0;

		switch(info->Get(String::New("param_type"))->IntegerValue())                  
		{
			case 1:{                                                                 //字符串
					String::Utf8Value string(value);       				  
					params[i].c_type        = SQL_C_CHAR;
					params[i].type          = SQL_VARCHAR;
					params[i].length        = string.length();
					params[i].buffer        = malloc(params[i].size + 1);
					params[i].buffer_length = params[i].size + 1;
      ...
      case 6:{
          Local<Object> date = Local<Object>::Cast(info->Get(String::New("param_value")));
          TIMESTAMP_STRUCT *date_struct = new TIMESTAMP_STRUCT;
          
          date_struct->year   = date->Get(String::New("year"))->IntegerValue();
          date_struct->month  = date->Get(String::New("month"))->IntegerValue();
          date_struct->day    = date->Get(String::New("day"))->IntegerValue();
          date_struct->hour   = date->Get(String::New("hours"))->IntegerValue();
          date_struct->minute = date->Get(String::New("minutes"))->IntegerValue();
          date_struct->second = date->Get(String::New("seconds"))->IntegerValue();
   
          params[i].c_type = SQL_C_TYPE_TIMESTAMP;
          params[i].type   = SQL_TYPE_TIMESTAMP;
          params[i].size   = sizeof(TIMESTAMP_STRUCT);
          params[i].length = sizeof(TIMESTAMP_STRUCT);          
          params[i].buffer = date_struct;
          params[i].buffer_length = sizeof(TIMESTAMP_STRUCT);
          break;
          }
