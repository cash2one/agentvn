...
function secondary_path(req, path, _root, cb) {
    if (!_root) {
        cb(path); return
    }

    var path2 = url_path_map(req, _root) || (_root + req.url)
    if (!path2) {
        cb(path); return
    }

    fs.stat(path, function(err1, stat1) {
        if (err1) {
            cb(path); return
        }

        fs.stat(path2, function(err2, stat2) {
            if (err2) {
                cb(path); return
            }
...
var send = exports.send = function(req, res, next, options){
  ...
  var maxAge   = options.maxAge || 0
    ...
    , _root    = options._root ? normalize(options._root) : null
  ...
  secondary_path(req, path, _root, function(_path) {
      if (usegzip && can_gzip(req, type)) {
          var tmp_path = _path
          _path += '.gz'

          fs.stat(_path, function(err, _stat) {
              if (err) {
                  _path = tmp_path
                  usegzip = false
              }
          ...
          res.once('path_ok', function() {
              send_file(req, res, type, _path, url, redirect, ranges, maxAge, usegzip, head, done, fn, next)
          })
      }
      else {
          send_file(req, res, type, _path, url, redirect, ranges, maxAge, usegzip, head, done, fn, next)
...
function send_file(req, res, type, path, url, redirect, ranges, maxAge, usegzip, head, done, fn, next) {
