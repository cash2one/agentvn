...
NODE_SET_METHOD(target, "create_captcha", CreateCaptcha);
...
//--------------------------------------------------------------------------------------------------------
static Handle<Value> CreateCaptcha (const Arguments& args)
{
    HandleScope scope;  
    REQ_ARGS(6);
    REQ_INT_ARG(0, width);
    ...
    info->font = new char[font.length()+1];
    strcpy(info->font, *font);

    info->code = new char[code.length()+1];
    strcpy(info->code, *code);

    eio_custom(EIO_Create, EIO_PRI_DEFAULT, EIO_AfterCreate, info);
...
//--------------------------------------------------------------------------------------------------------
static void  EIO_Create(eio_req *req) {
    CaptchaInfo *info = (CaptchaInfo *)(req->data);
    
    int code_count  = strlen(info->code);
    int unit_width  = info->width/(code_count*1.5);
    int unit_height = info->height;   
    int level       = info->level;

    int *direct = new int[code_count];
    gdImagePtr *unit_images = new gdImagePtr[code_count];
    ...
    for(int i=0; i<code_count; i++) {
        ...
        unit_images[i] = gdImageCreateTrueColor(unit_width, unit_height);

        back_color = gdImageColorAllocate(unit_images[i], 255, 255, 255);

        gdImageFilledRectangle(unit_images[i], 0, 0, unit_width, unit_height, back_color);
 
        angle = (level) * 3.14 / 40;
    
        direct[i] = rand()%100>=50 ? 1 : -1;

        if (direct[i] >= 0 ) {
            x_offset = int(unit_width*level/25);             //逆时针
            y_offset = int((35+level)*unit_height/50);
        }
        else {
            x_offset = int(unit_width/(level*2+20));
            y_offset = int(15*unit_height/(20+level));
        }

        gdImageStringFT( unit_images[i], NULL, random_color(), info->font, int(2*unit_width/3), 
                         angle*direct[i], x_offset, y_offset, code);
    }    

    gdImagePtr final_image = gdImageCreateTrueColor(info->width, info->height);

    back_color  = gdImageColorAllocate(final_image, 255, 255, 255);

    gdImageFilledRectangle(final_image, 0, 0, info->width, info->height, back_color);
    ...
    for(int k=0; k<code_count; k++) {
        int unit_weight = int(info->width/(code_count*1.5));
        int left_edge   = int((1.2*k+0.8)*unit_weight);
        double height = double(info->height);
        double width  = double(info->width);

        for(int i=0; i<unit_weight; i++) {
            for(int j=0; j<info->height; j++) {
                rgb = gdImageGetPixel(unit_images[k],i,j);
                if(rgb != white ) {
                    point = int( i+direct[k]*sin((j/height*2-0.5)*3.14)*width/(80-level*6) );
                    // point = int(i+direct[k]*(sqrt(4*j*height/5 - j*j)-unit_weight/15));
                    gdImageSetPixel(final_image, left_edge+point, j, rgb);

                }
            }
        }   

        gdImageDestroy(unit_images[k]);  
    }
    ...
    for (int i = 0; i < 5 ; i++) {
      for (int j = 0; j < 2 ; j++) {   
        for(int count = 0; count < 10; count++) {
          int x = rand()%int(info->width/5) + i*int(info->width/5);
          int y = rand()%int(info->height/2) + j*int(info->height/2);
          gdImageFilledEllipse (final_image, x, y, int(info->width/100), int(info->width/100), rand()%white);
          // gdImageSetPixel(final_image, x, y, rand()%white);
        }
      }
    }   
    ...
    int size;
    char *data = (char*)gdImagePngPtrEx(final_image, &size, 1);

    info->data = data;
    info->data_size = size;

    gdImageDestroy(final_image);
    delete[] direct;
    delete[] unit_images;
} 
//--------------------------------------------------------------------------------------------------------
static int  EIO_AfterCreate(eio_req *req) {
  ...
  argv[0] = Local<Value>::New(Encode(info->data, info->data_size));
...
static int  random_color() {
    int red   = rand()%155+100;
    int green = rand()%50;
    int black = rand()%155+100;

    return (red * 256 * 256) + (green * 256) + black;
}
//--------------------------------------------------------------------------------------------------------
